...
@AILayout(R.layout.activity_que_history_fragment)
public class QueHistoryFragmentActivity extends BaseFragment implements OnRapidFloatingButtonSeparateListener
    ...
    public void init() {
        ...
        SOSList = (ListView)findViewById_(R.id.HisQue);
...
//        }
    ...
    public void changewha(int wha_) {
        wha = wha_;
        eventCache = Que.getRemoteTitleList(0, wha);
        setList();
    }
    ...
    public void setList(){
        Que = new HistoryAdapter(getActivity(), user_id, wha, 0, eventCache);
        SOSList.setAdapter(Que);

        events = Que.getEventList();
